...
print(float(count)/len(entity2id.keys())) #0.5921717171717171 有（）的实体比例
print(float(count1)/float(count)) #0.6135986733001658 有（）的实体中，（）部分不在原实体列表中的比例
...
for id in range(len(id2entity.keys())):
    ...
    if "(" in entity_str and ")" in entity_str:###100%都有（）
        count += 1
        begin = entity_str.find('(')
        end = entity_str.find(')')
        w1 = entity_str[:begin].strip()
    ...
    # else:
    #     eid2wid.append([str(entity2id[entity_str]), "2", str(word2id[entity_str])])  #全都有（）所以不单独添加2关系
...
ROLE_DICT = {
    "Countries/Regions": "1",  # 如 EGYPT, NIGERIA, USA, CHINA 等国家/地区名称
    "Government Entities": "2",  # GOVERNMENT, PRIME MINISTER, PARLIAMENT, 政府相关结构/职务
    "Political Groups/Parties": "3",  # OPPOSITION PARTY, RULING PARTY, DEMOCRATIC PARTY
    "Military & Armed Groups": "4",  # ARMY, MILITIA, REBEL, TERRORIST GROUP
    "Law & Judiciary": "5",  # JUDGE, COURT, SUPREME COURT, ATTORNEY, MAGISTRATE
    "Police & Security": "6",  # POLICE, SECURITY OFFICER, BORDER PATROL
    "Media": "7",  # MEDIA, NEWSPAPER, BLOG, JOURNALIST, TV
    "Business & Corporations": "8",  # COMPANY, CORPORATION, INVESTOR, TANKER, NETFLIX
    "NGOs/International Orgs": "9",  # UNITED NATIONS, RED CROSS, WORLD BANK, NGO
    "Religious Entities": "10",  # ISLAM, CHRISTIANITY, PRIEST, CLERIC, VATICAN
    "Healthcare": "11",  # HOSPITAL, DOCTOR, SURGEON, HLH 标签的实体
    "Education": "12",  # SCHOOL, UNIVERSITY, STUDENT, PROFESSOR
    "Individuals/Leaders": "13",  # 人名，如 OBAMA, HILLARY CLINTON, MAHMOUD ABBAS
    "Ethnic/Religious Groups": "14",  # JEWISH, SUNNI, SHIA, MUSLIM, TAMIL
    "Civilians & Residents": "15",  # RESIDENT, CITIZEN, CIVILIAN, MIGRANT
    "Criminal Entities": "16",  # CRIMINAL, GANG, ROBBER, BURGLAR
    "Environmental Orgs/Tags": "17",  # 用于 ENV 标签或环保相关，如 CONSERVATIONIST
    "Transportation & Infrastructure": "18" , # AIRLINE, AIRPORT, PORT, TANKER
    "Unknown": "19"
}
...
def llm_get_relation(w,n):
    ...
    prompt = (
        ...
        f"2.Government Entities\n"
        f"3.Political Groups/Parties\n"
        f"4.Military & Armed Groups\n"
        f"5.Law & Judiciary\n"
        f"6.Police & Security\n"
        f"7.Media\n"
        f"8.Business & Corporations\n"
        f"9.NGOs/International Organizations\n"
        f"10.Religious Entities\n"
        f"11.Healthcare\n"
        f"12.Education\n"
        f"13.Individuals/Leaders\n"
        f"14.Ethnic/Religious Groups\n"
        f"15.Civilians & Residents\n"
        f"16.Criminal Entities\n"
        f"17.Environmental Orgs/Tags\n"
        f"18.Transportation & Infrastructure\n"
        f"19.Unknown\n"
        f"输入的实体名称为：{w}，一共包含{n}个实体：，这些实体名称全来至 GDELT 数据集（包括全球事件、语言与情绪数据库）。\n"
        ...
        for k in entity_chunk:##############定义从那个片段开始处理
            ...
            if i == patch or k == entity_chunk[-1]:
                ...
                while True:
                    ...
                    if h > 2 and len(missing_entities) <5:
                        complete_json = {}
                ...
                for k_j, v_j in json_text.items():
                    try:#
                        ...
                        if ent != str(global_entity_idx):#LLM返回顺序错误，需要重新调用
