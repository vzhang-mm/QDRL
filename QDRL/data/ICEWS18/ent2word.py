...
print(float(count)/len(entity2id.keys()))   #0.6463335214691964   有（）的实体比例
print(float(count1)/float(count))  #0.002149526432457849   有（）的实体中，（）部分不在原实体列表中的比例
            ...
            if i == patch or k == entity_chunk[-1]:
                ...
                for k_j, v_j in json_text.items():
                    try:#
                        ...
                        if ent != str(global_entity_idx):#顺序错，不影响结果
